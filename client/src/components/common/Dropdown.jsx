...
export function Dropdown({
  ...
  return (
    <div className={`dropdown ${className}`} ref={dropdownRef}>
      ...
      >
        <span className={`dropdown-value ${!value ? 'placeholder' : ''}`}>{selectedLabel}</span>
          ...
          <div className="dropdown-options">
            {Object.entries(groupedOptions).map(([group, groupOptions]) => (
              <div key={group} className="dropdown-group">
                {groupBy && group !== 'all' && <div className="dropdown-group-label">{group}</div>}
            ...
            {filteredOptions.length === 0 && <div className="dropdown-empty">No results found</div>}
          </div>
        </div>
      )}
    </div>
