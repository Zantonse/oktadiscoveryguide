...
import { areaToProductMap, getProductByName, aiSecurityProducts } from '../../data/aiSecurityProducts.js';
...
export function ReportCard() {
  const { reportCard, interestLevel, resetSession, selectedTrack, discoveredAreas, discoveryProgress, phase, messages } = useSession();
      ...
      <div className="report-card">
        ...
        {solutionBridgingAnalysis && (
          <div className="report-section solution-bridging">
              ...
              <div className="solution-bridging-details">
                ...
                <div className="products-mentioned">
                      ...
                      {solutionBridgingAnalysis.relevantMentioned.map(name => {
                        ...
                        return (
                          <span key={name} className="product-mentioned-badge relevant">
                            <span className="product-dot" style={{ backgroundColor: product?.color || '#10b981' }} />
                            {name}
                          </span>
                        );
                      })}
                  ...
                  {solutionBridgingAnalysis.irrelevantMentioned.length > 0 && (
                    <>
                      <span className="products-mentioned-label" style={{ marginLeft: '8px' }}>Off-topic:</span>
                      {solutionBridgingAnalysis.irrelevantMentioned.map(name => {
                        const product = getProductByName(name);
                        return (
                          <span key={name} className="product-mentioned-badge irrelevant">
                            <span className="product-dot" style={{ backgroundColor: product?.color || '#ef4444' }} />
                            {name}
                          </span>
                        );
                      })}
            ...
            {solutionBridgingAnalysis.relevantProducts.length > 0 && solutionBridgingAnalysis.relevantMentioned.length === 0 && (
              <div className="solution-bridging-tip">
                <strong>Tip:</strong> Based on discovered pain points, relevant products were: {solutionBridgingAnalysis.relevantProducts.join(', ')}
              </div>
            )}
          </div>
        )}
