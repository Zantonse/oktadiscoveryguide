...
export function SuggestedQuestions({ inline = false }) {
    ...
    <div className="suggested-questions">
      ...
      {nextPriorityAreas.length > 0 && (
        <div className="priority-areas-hint">
          ...
          <span className="hint-areas">{nextPriorityAreas.slice(0, 2).map(a => a.replace(/_/g, ' ')).join(', ')}</span>
        </div>
      )}
  ...
  return (
    <Card title="Suggested Questions" className="suggested-questions-card" collapsible defaultCollapsed={true}>
      {content}
    </Card>
  );
}
