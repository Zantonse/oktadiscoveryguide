...
const technicalFlow = [
  {
    ...
    actions: ['Show technical understanding', 'Acknowledge their expertise', 'Focus on solving problems'],
        ...
        <div className="flow-modal-content">
          <div className="flow-timeline">
            {flow.map((step, index) => {
              ...
              return (
                <div key={step.phase} className={`flow-timeline-step ${isFull ? 'discovered-full' : isPartial ? 'discovered-partial' : ''}`}>
                  <div className="flow-timeline-marker">
                    {isFull ? (
                      ...
                      <span className={`flow-step-num ${isPartial ? 'partial' : ''}`}>{index + 1}</span>
                    )}
                    {index < flow.length - 1 && <div className={`flow-timeline-line ${isFull ? 'discovered' : ''}`} />}
                  </div>
